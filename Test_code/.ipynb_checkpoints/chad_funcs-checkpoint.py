...
def stab_cor(stab_meth, z, z0, MO_tow, dtindex):
    ...
    if pd.isnull(stab_meth): 
        Psi = np.zeros(shape = len(dtindex)) #Psi is zeros if 
        ...
        Psi = Psi.map(lambda x: 0 if pd.isnull(x) == True else x)
...
def wind_prof(tow, stab_meth, WS_use, MO_tow, dtindex, step = 0.1, z_Umes = 'use_mes'):
    ...
    for z in z_range: 
        WS_prof[z] = WS_of_z(stab_meth, z, z0, d, h_veg, z0_soil, u_of_h, ustar_calc, ustar_soil, MO_tow, dtindex)
...
def WS_of_z(stab_meth, z, z0, d, h_veg, z0_soil, u_of_h, ustar_calc, ustar_soil, MO_tow, dtindex):
    
    if z >= h_veg:
        Psi = stab_cor(stab_meth, z, z0, MO_tow, dtindex)
        WS_z = ustar_calc/k*np.log(((z-d)/z0) + Psi)
    elif z>=z0:
        WS_z = u_of_h*np.exp(z/h_veg - 1) 
    elif z>0:
