#This file has all the basic setup needed for various notebooks and py files used in Cheesehead advection project
#Includes: Imports for commonly used packages, Constants, setup of tc (var containing basic tower info), and data

#Imports and settings
import netCDF4 as nc
import numpy as np
import matplotlib.pyplot as plt
import pandas as pd
import datetime as dt
import cdflib
import scipy.stats
from scipy.interpolate import RBFInterpolator
import pickle
import matplotlib as mpl
import scipy
from pysolar.solar import*
import seaborn as sns



#Use this to ignore warning in inerpolation script, maybe get rid of later to check for other issues
from warnings import simplefilter
simplefilter(action="ignore", category=pd.errors.PerformanceWarning)

pd.set_option('display.max_columns', None) #Change settings in Pandas so it shows all columns
 
idx = pd.IndexSlice #Used for slicing dataframes
    


    
#Constants
#Met
R = 8.314 #kgm^2s^-2K^-1mol^-1
L = 2.5*10**3 #Latent heat of vapoization of water at 0 deg C [J/g], should try to calc based on dependency on C
cp = 1005 #J/kg*K
mm = 0.02896 #kg/mol molar mass dry air
mm_w = 18.01 # molar mass of water [g/mol]
CtoK = 273.15 #Conversion from Celcius to Kelvin
k = 0.4 #von Karmann constant
g = 9.81 #[m/s^2] gravitational constant
# a_default = 1 #attenuation coeficient for in-canoy flow, From Campbell and Norman table 5.2 for Larch trees

   
    
    
#Tower info   
tc = pd.read_csv('../Inputs/tower_coords.csv', index_col = 'Tower') #datafrane of tower coordinates

#Convert lat and lon to coordinates in km, origin currently set to PFA coordinates, should change this since PFk is out of range #fixthis?
tc['x'] = (tc.Lon - tc.Lon['PFA'])*111*np.cos(tc.Lat['PFA']*np.pi/180)*1000
tc['y'] = (tc.Lat - tc.Lat['PFA'])*111*1000

mth = 33 #Mean top level height, 32.62 rounded up, includes PFA, b, e, g, h , i, j, k, m, n, p, q, t
mmh = 11.5 #Mean mid-level height, 11.59 rounded up, includes PFb, c, g, h, i, j, k, l, m, n, p, q, s, t
mlh = 2 #Mean lower-level height (don't know this precisely but most towers are listed at 2)


#Read in run cases
cases = pd.read_csv('../Inputs/cases_limited.csv', index_col = 'case') #dataframe of tower coordinates


#Output base folder filepath
input_filepath = '../Inputs/'
intermed_filepath = '../Intermed_data/'
output_filepath = '../Outputs/'

'''---------------------------------------------------------------------------------------'''



#Function to read in universal data
#Reads in all data unless start and end date are specified
def readdata(var, startdate = dt.datetime(2019, 6, 20), enddate = dt.datetime(2019, 10, 13, 23, 30)):
    unidata_paths = {
        'TA':'../Inputs/TA.pickle',
        'H2O':'../Inputs/H2O.pickle',
        'hrz_wind': '../Inputs/hrz_wind.pickle',
        'PA_top': '../Inputs/PA_top.pickle',
        'MO': '../Inputs/MO_length.pickle',
        'zeta': '../Inputs/zeta.pickle',
        'w_son': '../Inputs/w_son.pickle',
        'w_cont':'../Inputs/w_cont.pickle',
        'w_cont_lai':'../Inputs/w_cont_lai.pickle',
        'dtindex': '../Inputs/dtindex.pickle'} #Not used currently but could add back in if I can figure out how to subset
    
    if var == 'PA_top' or var =='vert_wind_sonic':
        #No levels for PA, dtindex, select start to end date
        data = pd.read_pickle(unidata_paths[var]).loc[startdate:enddate] 
    elif var == 'dtindex':
        full_dtindex = pd.read_pickle(unidata_paths[var]) #read in full dtindex
        data = full_dtindex[(full_dtindex>=startdate) & (full_dtindex <= enddate)] #cut to desired times
    
    else:
        #Select all levels, start to end date, all tows
        #data = pd.read_pickle(unidata_paths[var]).loc[idx[:, startdate:enddate], :] 
        data = pd.read_pickle(unidata_paths[var]).loc[startdate:enddate] 
        

    return data


def make_multi_df(col_lists, level_names, index):
    # dtindex = readdata('dtindex', startdate, enddate)
    col_ind = pd.MultiIndex.from_product(col_lists, names=level_names)
    df = pd.DataFrame(index = index, columns = col_ind, dtype = 'float')
    return df




NSEWdirlist = ['N', 'S', 'E', 'W'] #list of directions
c2_dirlist = ['NS','EW'] #Cardinal 2 dir (N/S, E/W list)
u2_dirlist = ['up', 'down'] #Up/downwind 2 dir list
trac_levlist = [2, 10, 30] #Measurement levels, these are just short-hand, actual levels are closer to 2, 12, and 33
trac_varlist = ['TA', 'H2O'] #Energy tracer variables
dimlist = ['x', 'y']
U_varlist = ['u', 'v']
U_levlist = [33]


towlist = tc.index #List of towers


hh_index = np.arange(0, 24, 0.5) #half-hourly index

#Multiindices for dataframes, names give nesting order 
varlevtow4dir_cols = pd.MultiIndex.from_product([trac_varlist, trac_levlist, towlist, NSEWdirlist], names=["var", "lev", "tow", "direct"])
varlevtow_cols = pd.MultiIndex.from_product([trac_varlist, trac_levlist, towlist], names=["var", "lev", "tow"])
varlev_cols = pd.MultiIndex.from_product([trac_varlist, trac_levlist], names=["var", "lev"])
tow4dir_cols = pd.MultiIndex.from_product([towlist, NSEWdirlist], names=["tower", "direct"]) #for columns of interp data df
varlevtowc2dir_cols = pd.MultiIndex.from_product([trac_varlist, trac_levlist, towlist, c2_dirlist], names=["var", "lev", "tow", "direct"])
towc2dir_cols = pd.MultiIndex.from_product([towlist, c2_dirlist], names=["tower", "direct"]) 
towu2dirdim_cols = pd.MultiIndex.from_product([towlist, u2_dirlist, dimlist], names=["tow", "direct", "dim"]) #for updown corrds, into dict of directions


'-----------------------------------------------------------------------------------------------------------'
#color list
colorlist = ["xkcd:magenta", "xkcd:blue green", "xkcd:grey", "xkcd:sky blue", "xkcd:light orange",
             "xkcd:grey blue", "xkcd:dark yellow", "xkcd:pale purple", "xkcd:coral",
             "xkcd:ocean blue", "xkcd:light olive", "xkcd:reddish brown", "xkcd:dark lavender", "xkcd:forrest green", 
             "xkcd:terracotta", "xkcd:light cyan", "xkcd:orchid", "xkcd:puke yellow", "xkcd:periwinkle blue"]
# mpl.rcParams['axes.prop_cycle'] = mpl.cycler(color=colorlist) 